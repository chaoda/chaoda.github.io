$(document).ready(function(){
	...
	var len = $("#content-imgs ul li").length; //获取焦点图个数
	var index = 0;
	var picTimer;
	...
	$("#content-imgs").append(btn);
	$("#content-imgs .btnBg").css("opacity",0.5);

	//为小按钮添加鼠标滑入事件，以显示相应的内容
	$("#content-imgs .btn span").css("opacity",0.4).mouseover(function() {
		index = $("#content-imgs .btn span").index(this);
		showPics(index);
	}).eq(0).trigger("mouseover");

	//本例为左右滚动，即所有li元素都是在同一排向左浮动，所以这里需要计算出外围ul元素的宽度
	$("#content-imgs ul").css("width",sWidth * (len));
	
	//鼠标滑上焦点图时停止自动播放，滑出时开始自动播放
	$("#content-imgs").hover(function() {
